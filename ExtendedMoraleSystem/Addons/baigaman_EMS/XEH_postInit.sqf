...
logicInterval = GVAR(logicInterval); // Interval to check for new groups to add the surrender logic to
moraleBreakingPoint = GVAR(moraleBreakPoint); // Threshold the unit need to pass to test for morale
moraleCheckCooldown = GVAR(moraleCheckCooldown); // How much time has to pass between morale checks
useMoraleDebuff = GVAR(useMoraleDebuff); // Use morale debuff mechanic. Try setting this to false if units flee or surrender too often.
fightingRetreatDuration = GVAR(fightingRetreatDuration); // How long will the fighting retreat last?
aiIgnoreFleeing = GVAR(aiIgnoreFleeing); // Should the AI ignore fleeing units?
fleeMinDistance = GVAR(fleeMinDistance); // Minimum distance an enemy will flee
fleeMaxDistance = GVAR(fleeMaxDistance); // Max distance an enemy will flee
enableAIGoHostile = GVAR(enableAIGoHostile); // Enable the AI to pick up weapons and fight back if not handcuffed
goHostileInterval = GVAR(goHostileInterval); // Interval to run the 'go hostile' check
enableAddCableTies = GVAR(enableAddCableTies); // Add cable ties to units
cableTieAmountPlayers = GVAR(cableTieAmountPlayers); // Amount of cable ties to add to each player
cableTieAmountAI = GVAR(cableTieAmountAI); // Amount of cable ties to add to each AI unit
enableRally = GVAR(enableRally); // Enable players to rally surrendered or fleeing units
cleanUpInterval = GVAR(cleanUpInterval); // Interval to check for units to clean up
cleanUpDistance = GVAR(cleanUpDistance); // Run cleanup on units that are this distance or further from nearest player
...
fnc_addRallyAction = {
	...
	if (_unit getVariable ["action_added", false]) exitWith {};

	_unit setVariable ["action_added", true];
